...
import { Package, Calendar, Edit3, Trash2, AlertTriangle, UserCheck, MessageSquare, Search, Sparkles, Mic, Sun, FileText } from 'lucide-react';
import { formatDateIT } from '../utils/dateUtils';

// 📱 ARRAY NUMERI DI TELEFONO SOLE 365
const numeriSole365 = [
  '3284344912', // Marco Costantino
  '181208998756424', // Salvatore 
];
...
const SoleOrderCard = ({ ord, onConfirmOrder, onEditOrder, onDeleteOrder, deleteConfirmId, setDeleteConfirmId }) => {
    ...
    <div
      className={`petruzzi-card p-6 rounded-3xl border-2 transition-all flex flex-col lg:flex-row items-stretch justify-between gap-6 shadow-md hover:shadow-lg ${
        ord.da_verificare_manualmente
          ...
          : 'border-amber-300/80 bg-[#FFFDF9] hover:border-amber-400'
      }`}
    >
      <div className="w-full lg:w-64 shrink-0 space-y-3 pb-4 lg:pb-0 border-b lg:border-b-0 lg:border-r border-amber-200 lg:pr-6 flex flex-col justify-between">
        <div>
          <div className="flex items-center justify-between lg:block">
            <h3 className="font-black text-amber-950 text-xl tracking-tight leading-snug">
          ...
          <div className="space-y-2 mt-3">
            <div className="space-y-1">
              <div className="flex items-center space-x-1.5 text-[11px] text-amber-700 font-medium">
                ...
                <span>Arrivo su WhatsApp: <strong className="text-amber-900">{formatDateIT(ord.data_ricezione) || 'N/D'}</strong></span>
              </div>
            </div>
            <div className="inline-block mt-1 text-xs font-black text-amber-900 bg-amber-100 px-2.5 py-1 rounded-lg border border-amber-300">
      ...
      <div className="flex-1 space-y-3 min-w-0">
        <div className="flex items-center justify-between">
          <span className="text-xs font-black text-amber-800 uppercase tracking-wider">Articoli Sole 365:</span>
          <span className="text-[11px] font-extrabold text-amber-700 bg-amber-100 px-2 py-0.5 rounded-full border border-amber-300">
            {prodotti ? prodotti.length : 0} voci
          </span>
        </div>

        <div className="bg-amber-50/90 rounded-2xl p-3 border border-amber-300/80 divide-y divide-amber-200/80 shadow-inner max-h-64 overflow-y-auto">
        ...
        {ord.note_ordine && (
          <div className="bg-red-50/90 p-3 rounded-xl border border-red-300 text-xs text-red-950">
            <div className="flex items-center space-x-1.5 font-black text-red-900 mb-0.5 text-[11px]">
      ...
      <div className="w-full lg:w-56 shrink-0 pt-4 lg:pt-0 border-t lg:border-t-0 lg:border-l border-amber-200 lg:pl-6 flex flex-col justify-center space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          >
            <Edit3 className="w-4 h-4 mr-1 text-petruzzi-300" /> Modifica
          </button>
...
export default function OrdiniSole({ ordini, selectedDate, setSelectedDate, onEditOrder, onDeleteOrder, onConfirmOrder }) {
  const [deleteConfirmId, setDeleteConfirmId] = useState(null);

  const ordiniSole = ordini.filter(o => {
    const mittenteStr = (o.mittente || '').toLowerCase();
    const isSoleByName = mittenteStr.includes('sole') || mittenteStr.includes('365');
    const isSoleByPhone = numeriSole365.some(numero => mittenteStr.includes(numero));
    const isSole = isSoleByName || isSoleByPhone;
    const matchesDate = !selectedDate || o.data_consegna === selectedDate;
    
    return isSole && matchesDate;
  });

  return (
    <div className="space-y-6 animate-fadeIn">
      
      {/* Toolbar Aggiornata con pulsante PDF Sole */}
      <div className="flex flex-wrap items-center justify-between gap-3 p-3 px-4 rounded-xl bg-amber-100/90 border border-amber-200">
        <div className="flex items-center space-x-3">
          <Sun className="w-5 h-5 text-amber-600" />
          <span className="text-xs font-black text-amber-900 uppercase">Ordini Gruppo Sole 365</span>
          <div className="flex items-center space-x-1.5 ml-4">
            <input
              type="date"
              value={selectedDate}
              onChange={(e) => setSelectedDate(e.target.value)}
              className="bg-white border border-amber-300 text-amber-950 rounded-lg px-2.5 py-1 text-xs font-bold focus:ring-1 focus:ring-amber-700 outline-none shadow-sm"
            />
          </div>
          <button
            onClick={() => setSelectedDate(new Date().toISOString().split('T')[0])}
            className={`px-3 py-1 rounded-lg text-xs font-bold border transition ${
              selectedDate === new Date().toISOString().split('T')[0] ? 'bg-amber-700 text-white border-amber-700' : 'bg-white text-amber-800 border-amber-300 hover:bg-amber-50'
            }`}
          >
            Oggi
          </button>
          <button
            ...
            className={`px-3 py-1 rounded-lg text-xs font-bold border transition ${
              !selectedDate ? 'bg-amber-700 text-white border-amber-700' : 'bg-white text-amber-800 border-amber-300 hover:bg-amber-50'
            }`}
          >
            Tutti
          </button>
        ...
        {/* Pulsante Stampa PDF per Sole 365 */}
        <a
          href={`/api/pdf/sole?data=${selectedDate}`}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center space-x-2 px-3.5 py-1.5 rounded-lg bg-amber-700 hover:bg-amber-800 text-white text-xs font-bold transition transform active:scale-95 shadow-sm"
          title="Stampa Scheda PDF per i clienti Sole 365"
        >
          <FileText className="w-4 h-4" />
          <span>Stampa PDF Sole</span>
        </a>
      </div>

      <div className="space-y-4">
        {ordiniSole.length === 0 ? (
          <div className="text-center py-16 petruzzi-card rounded-3xl border border-amber-200 bg-amber-50/50 space-y-3">
            <Sun className="w-16 h-16 text-amber-400 mx-auto" />
            <h3 className="text-xl font-extrabold text-amber-800">
              Nessun ordine Sole 365 per questa data.
            </h3>
          </div>
        ) : (
          ordiniSole.map((ord) => (
