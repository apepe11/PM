    ...
    >
          ...
          <div className="space-y-2 mt-3">
            <div className="space-y-1">
              <div className="flex items-center space-x-1.5 text-[11px] text-petruzzi-700 font-medium">
                ...
                <span>Arrivo su WhatsApp: <strong className="text-petruzzi-900">{formatDateIT(ord.data_ricezione) || 'N/D'}</strong>[cite: 20]</span>
              </div>
              <div className="flex items-center space-x-1.5 text-[11px] text-petruzzi-700 font-medium">
                <Sparkles className="w-3.5 h-3.5 text-petruzzi-600 shrink-0" />
                <span>Elaborato dal sistema: <strong className="text-petruzzi-900">{ord.timestamp_elaborazione ? formatDateIT(ord.timestamp_elaborazione) : (formatDateIT(ord.data_ricezione) || 'N/D')}</strong>[cite: 20]</span>
              </div>
            </div>
            <div className="inline-block mt-1 text-xs font-black text-petruzzi-900 bg-petruzzi-100 px-2.5 py-1 rounded-lg border border-petruzzi-300">
              Data Consegna: {formatDateIT(ord.data_consegna)}[cite: 20]
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 space-y-3 min-w-0">
        <div className="flex items-center justify-between">
          <span className="text-xs font-black text-petruzzi-800 uppercase tracking-wider">Articoli Ordinati e Organizzati:[cite: 20]</span>
          <span className="text-[11px] font-extrabold text-petruzzi-700 bg-petruzzi-100 px-2 py-0.5 rounded-full border border-petruzzi-300">
            {prodotti ? prodotti.length : 0} voci[cite: 20]
          </span>
        </div>
        ...
        <div className="bg-petruzzi-50/90 rounded-2xl p-3 border border-petruzzi-300/80 divide-y divide-petruzzi-200/80 shadow-inner max-h-64 overflow-y-auto">
          {prodotti && prodotti.length > 0 ? (
              ...
              return (
                <div key={gruppo.key} className="py-3 flex flex-col xl:flex-row xl:items-end justify-between gap-3 first:pt-0 last:pb-0">
                  <div className="flex-1 min-w-0 mb-1">
                    <span className="font-extrabold text-petruzzi-950 text-sm truncate block">{primo.nome_articolo || primo.codice_articolo}[cite: 20]</span>
                    {primo.is_peso_fisso && (
                      <span className="inline-block mt-1 text-[9px] bg-petruzzi-200 text-petruzzi-800 px-1.5 py-0.5 rounded font-mono uppercase">Peso Fisso ({primo.peso_unitario_kg} KG)[cite: 20]</span>
                    )}
                    {primo.pezzi_totali && (
                      <span className="inline-block mt-1 text-[9px] bg-petruzzi-100 text-petruzzi-700 px-1.5 py-0.5 rounded font-mono uppercase ml-1">Pezzo {primo.pezzo_index} di {primo.pezzi_totali}[cite: 20]</span>
                    )}
                    {!gruppo.eccezione && gruppo.items.length > 1 && (
                      <span className="inline-block mt-1 text-[9px] bg-petruzzi-100 text-petruzzi-700 px-1.5 py-0.5 rounded font-mono uppercase ml-1">{gruppo.items.length} voci · stesso lotto[cite: 20]</span>
                    )}
                  </div>

                  <div className="flex items-end gap-2 shrink-0 flex-wrap justify-end">
                    <div className="flex flex-col items-start">
                      <label className="text-[9px] font-black text-petruzzi-600 uppercase mb-0.5 ml-0.5">Quantità[cite: 20]</label>
                      <div className="flex flex-wrap gap-1 justify-end max-w-[160px]">
                        {gruppo.items.map(({ prod, idx }) => (
                          <span key={idx} className="flex items-center justify-center h-[26px] font-black text-petruzzi-950 bg-white px-2.5 rounded-md border border-petruzzi-300 shadow-sm text-[11px] whitespace-nowrap">
                            {prod.quantita} {prod.unita_di_misura}[cite: 20]
                          </span>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col items-start">
                      <label className="text-[9px] font-black text-petruzzi-600 uppercase mb-0.5 ml-0.5">Lotto Art.[cite: 20]</label>
                      <input
                        type="text"
                    ...
                    {/* CAMPO GRAMMATURA SEMPRE MODIFICABILE E LIBERO */}
                    <div className="flex flex-col items-start">
                      <label className="text-[9px] font-black text-petruzzi-600 uppercase mb-0.5 ml-0.5">Grammatura[cite: 20]</label>
                      <input
                        type="text"
            ...
            <p className="text-xs text-petruzzi-600 italic">Nessun prodotto estratto dal testo.[cite: 20]</p>
          )}
        </div>
        ...
        {ord.testo_originale && !ord.testo_originale.includes('Inserimento Manuale') && (
          <div className="bg-blue-50/70 p-2.5 rounded-xl border border-blue-200 text-xs text-blue-900 mt-2">
            <div className="flex items-center space-x-1.5 font-bold text-blue-800 uppercase tracking-wider mb-1 text-[10px]">
              ...
              <span>Msg. WhatsApp Originale:[cite: 20]</span>
            </div>
            <p className="italic font-medium">"{ord.testo_originale.replace(/🎙️\s*\[VOCALE TRASCRITTO\]:\s*/g, '').replace(/\[Parser Locale di Riserva\]\s*/g, '').replace(/\[Integrazione\/Correzione\]:\s*/g, ' + ')}"[cite: 20]</p>
          </div>
        )}
        ...
        {ord.note_ordine && (
          <div className="bg-petruzzi-100/90 p-3 rounded-xl border border-petruzzi-300 text-xs text-petruzzi-950">
            <div className="flex items-center space-x-1.5 font-black text-petruzzi-900 mb-0.5 text-[11px]">
              ...
              <span>Note Consegna:[cite: 20]</span>
            </div>
            <p className="italic font-medium">{ord.note_ordine}[cite: 20]</p>
          </div>
        )}
      </div>
      ...
      <div className="w-full lg:w-56 shrink-0 pt-4 lg:pt-0 border-t lg:border-t-0 lg:border-l border-petruzzi-200 lg:pl-6 flex flex-col justify-center space-y-3">
        {ord.stato_ordine === 'CONFERMATO' ? (
          <div className="p-3 bg-emerald-100 border border-emerald-300 rounded-2xl text-center space-y-1 shadow-sm">
            <span className="flex items-center justify-center space-x-1.5 text-xs font-black text-emerald-900">
              ...
              <span>✅ ORDINE CONFERMATO[cite: 20]</span>
            </span>
            <span className="text-[10px] text-emerald-800 font-bold block">Inviato in Produzione[cite: 20]</span>
            <span className="text-[10px] font-mono font-bold block mt-1 text-emerald-900">Lotto Gen: {ord.numero_lotto || '-'}[cite: 20]</span>
          </div>
        ) : (
          <div className="space-y-2 bg-petruzzi-50/50 p-2 rounded-2xl border border-petruzzi-200">
            ...
            >
              ...
              <span>✅ CONFERMA ORDINE[cite: 20]</span>
            </button>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          >
            <Edit3 className="w-4 h-4 text-petruzzi-300" />[cite: 20]
          </button>
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 p-3 px-4 rounded-xl bg-petruzzi-100/90 border border-petruzzi-200">
        <div className="flex items-center space-x-3">
          ...
          <span className="text-xs font-bold text-petruzzi-900 uppercase">Filtro Data Ordini:[cite: 20]</span>
          <div className="flex items-center space-x-1.5">
            ...
            {selectedDate && (
              <span className="text-xs font-black text-petruzzi-900 bg-white px-2.5 py-1 rounded-lg border border-petruzzi-300 shadow-sm font-mono">
                {formatDateIT(selectedDate)}[cite: 20]
              </span>
            )}
          </div>
          <button
        ...
        <div className="flex items-center space-x-2">
          {onReprocessAll && (
            <button onClick={onReprocessAll} className="flex items-center space-x-2 px-3 py-1.5 rounded-lg bg-amber-100 hover:bg-amber-200 text-amber-950 text-xs font-bold border border-amber-300 transition shadow-sm">
              ...
              <span>🧠 Rielabora Tutti con IA[cite: 20]</span>
            </button>
          )}
          <a
          ...
          >
            <span>📄 Stampa PDF Generale[cite: 20]</span>
          </a>
          <button onClick={onOpenNewOrderModal} className="flex items-center space-x-2 px-3.5 py-1.5 rounded-lg bg-petruzzi-800 hover:bg-petruzzi-900 text-white text-xs font-bold transition transform active:scale-95 shadow-sm">
            ...
            <span>➕ Aggiungi Ordine Manuale[cite: 20]</span>
          </button>
        </div>
      </div>
        ...
        {filteredOrdini.length === 0 ? (
          <div className="text-center py-16 petruzzi-card rounded-3xl border border-petruzzi-200 space-y-3">
            ...
            <h3 className="text-xl font-extrabold text-petruzzi-800">
              {selectedDate ? `Nessun ordine filtrato per la data (${formatDateIT(selectedDate)})` : 'Nessun ordine presente nel sistema'}[cite: 20]
            </h3>
            {ordini.length > 0 && (
              <div className="pt-2 flex justify-center space-x-3">
                <button onClick={() => setSelectedDate('')} className="px-4 py-2 bg-petruzzi-800 text-white font-bold text-xs rounded-xl hover:bg-petruzzi-900 shadow transition">
                  📋 Mostra Tutti gli Ordini ({ordini.length})[cite: 20]
                </button>
              </div>
            )}
          </div>
